...
public class AllConcurrentTests {
	...
	public static void setUp() {
		...
		createFile("fizz-buzz-9.txt", generateFizzBuzz(9)); // Used by head, tail tests b/c I wanted file that has
															// exactly 10 lines.
		...
		createFile("real_uniq.txt", "aa\naa\naa\na\naa\na\naa\naa"); // Used by testRealUniq() because I wanted file
																		// that actually will test for real uniq
																		// behavior (and not allow old implementation to
																		// pass).
		createFile("small_ascii.txt", "abcdefghijklm !#$%&()*+,-./0123456789:;<=>?@[]^_`{|}~nopqrstuvwxyz");
		createFile("letters.txt", "AaBbCcDdEe");
	...
	public static void tearDown() throws Exception {
		// Lets add a method: createTemporaryFile that should append to a set
		// containing these files to be deleted.
		// That way, you won't have to add the file names to this list when u write new
		// tests in other files (that redirect for example).
		if (!AllConcurrentTests.DEBUGGING_MODE) {
			String[] files = { "folder-contents.txt", "hello.txt", "hello2.txt", "world.txt", "hello-world.txt",
					...
					"replTest2.txt", "replTest3.txt", "ascii.txt", "empty.txt", "fizz-buzz-10.txt", "fizz-buzz-9.txt",
					"real_uniq.txt", "small_ascii.txt", "letters.txt", "killTest1.txt", "killTest2.txt",
					"killTest3.txt", "killTest4.txt", "killTest5.txt", "killTest6.txt", "killTest7.txt",
					"killTest8.txt" };
